...
const MILESTONES = [
  { year: '2026', event: 'ThinkMindLabs founded in Gurugram & New York by enterprise veterans with 50+ years of combined industry experience.' },
  { year: '2026 Q1', event: 'OpenVoce.ai launched — world\'s fastest AI concierge agent. Zero to live in under 5 minutes.' },
  { year: '2026 Q1', event: 'Pravakta.ai launched — world\'s first AI Stack as a Service with 100+ pre-trained voice agents.' },
  { year: '2026 Q1', event: 'TalkBuy.ai goes live — conversational retail AI for eCommerce, D2C and quick commerce.' },
  { year: '2026 Q2', event: 'AgentForDoc.ai launched — clinical AI agent for hospitals and healthcare providers.' },
  { year: '2026+', event: 'TallyAgent, FinOps Agent, Rewards, Recognition in build. The lab never stops shipping.' },
]
  ...
  return (
    <>
      ...
      <section className="section" style={{ paddingTop: 28, paddingBottom: 0 }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 16 }} className="about-grid-4">
            {TEAM.map(t => (
              <div key={t.name} style={{ background: 'var(--color-bg-elevated)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-xl)', padding: '16px 18px' }}>
        ...
        <div className="container">
          <div className="two-col">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {MILESTONES.map((m, i) => (
                <div key={m.year} style={{ display: 'flex', gap: 24, paddingBottom: 28, paddingTop: i === 0 ? 0 : 28, borderBottom: i < MILESTONES.length - 1 ? '1px solid var(--color-border)' : 'none' }}>
      ...
      <section className="section" style={{ background: 'linear-gradient(to bottom, transparent, rgba(255,98,0,0.02), transparent)' }}>
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }} className="values-grid">
            {VALUES.map(v => (
              <div key={v.title} style={{ background: 'var(--color-bg-elevated)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-xl)', padding: '16px 18px', transition: 'border-color 0.3s ease' }}
              ...
              >
                ...
                <p style={{ fontSize: 15, fontWeight: 700, color: 'var(--color-text-primary)', marginBottom: 8, letterSpacing: '-0.01em' }}>{v.title}</p>
